...
public class ReLogoTurtle extends BaseTurtle{
	...
	/**
	 * Makes a number of new trees and then executes a set of commands on the
	 * created trees.
	 * 
	 * @param number
	 *            a number
	...
	public AgentSet<cows.relogo.Tree> hatchTrees(int number, Closure closure) {
		AgentSet<cows.relogo.Tree> result = new AgentSet<>();
		AgentSet<Turtle> createResult = this.hatch(number,closure,"Tree");
		for (Turtle t : createResult){
	 ...
	 */
	@ReLogoBuilderGeneratedFor("cows.relogo.Tree")
	public AgentSet<cows.relogo.Tree> hatchTrees(int number) {
		return hatchTrees(number,null);
	}
