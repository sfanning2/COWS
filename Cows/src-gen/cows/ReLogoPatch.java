...
public class ReLogoPatch extends BasePatch{
	...
	/**
	 * Sprouts (makes) a number of new trees and then executes a set of commands on the
	 * created trees.
	 * 
	 * @param number
	 *            a number
	...
	public AgentSet<cows.relogo.Tree> sproutTrees(int number, Closure closure) {
		AgentSet<cows.relogo.Tree> result = new AgentSet<>();
		AgentSet<Turtle> createResult = this.sprout(number,closure,"Tree");
		for (Turtle t : createResult){
	 ...
	 */
	@ReLogoBuilderGeneratedFor("cows.relogo.Tree")
	public AgentSet<cows.relogo.Tree> sproutTrees(int number) {
		return sproutTrees(number,null);
	}

	/**
	 * Returns an agentset of trees from the patch of the caller.
	 * 
	 * @return agentset of trees from the caller's patch
	 */
	@ReLogoBuilderGeneratedFor("cows.relogo.Tree")
	public AgentSet<cows.relogo.Tree> treesHere(){
	  Grid grid = getMyObserver().getGrid();
	  GridPoint gridPoint = grid.getLocation(this);
	  AgentSet<cows.relogo.Tree> result = new AgentSet<cows.relogo.Tree>();
	  for (Turtle t : Utility.getTurtlesOnGridPoint(gridPoint,getMyObserver(),"tree")){
			if (t instanceof cows.relogo.Tree)
			result.add((cows.relogo.Tree)t);
		}
		return result;
	...
	/**
	 * Returns the agentset of trees on the patch at the direction (ndx, ndy) from the
	 * caller.
	 * 
	 * @param nX
	 *            a number
	 * @param nY
	 *            a number
	 * @returns agentset of trees at the direction (nX, nY) from the caller
	 */
	@ReLogoBuilderGeneratedFor("cows.relogo.Tree")
	public AgentSet<cows.relogo.Tree> treesAt(Number nX, Number nY){
		double dx = nX.doubleValue();
		double dy = nY.doubleValue();
		double[] displacement = {dx,dy};
	...
	/**
	 * Sprouts (makes) a number of new fences and then executes a set of commands on the
	 * created fences.
	 * 
	 * @param number
	 *            a number
	...
	public AgentSet<cows.relogo.Fence> sproutFences(int number, Closure closure) {
		AgentSet<cows.relogo.Fence> result = new AgentSet<>();
		AgentSet<Turtle> createResult = this.sprout(number,closure,"Fence");
		for (Turtle t : createResult){
	 ...
	 */
	@ReLogoBuilderGeneratedFor("cows.relogo.Fence")
	public AgentSet<cows.relogo.Fence> sproutFences(int number) {
		return sproutFences(number,null);
	}
